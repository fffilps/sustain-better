...
const WeatherApiButton = (props: Props) => {
  ...
  const fetchData = async () => {
    ...
    for (let x = 0; x < 10; x++) {
      setPathDescriptions((pathDescription) => [
        ...pathDescription,
        response.features[x].properties.headline,
      ]);
  ...
  return (
    <main className="py-2">
      <div>
        <button
          className="bordered rounded-lg bg-gray-500 p-4"
          onClick={fetchData}
        >
          API Call
        </button>
        {pathDescriptions && pathDescriptions.length && (
          <div className="flex flex-col gap-2 divide-y-4 py-2 justify-center items-center">
